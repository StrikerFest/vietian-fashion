...
export default function CartItemList({ cartItems, updateQuantity, removeFromCart }) {
    return (
        <div className="space-y-4">
            {cartItems.map(item => (
                <div key={item.id} className="flex items-center bg-gray-800 p-4 rounded-lg border border-gray-700 shadow-sm transition-transform hover:scale-[1.01]">
                    {/* Product Image */}
                    ...
                    <div className="flex-grow min-w-0 mr-4">
                        ...
                        <div className="text-sm text-gray-400 mt-1 flex items-center gap-2">
                            <span className="bg-gray-700 px-2 py-0.5 rounded text-xs">{item.size}</span>
                            <span className="bg-gray-700 px-2 py-0.5 rounded text-xs">{item.color}</span>
                        </div>
                    ...
                    <button
                        ...
                        className="ml-6 text-gray-500 hover:text-red-500 transition-colors p-2"
                        title="Remove Item"
