...
import { createClientComponentClient } from '@supabase/auth-helpers-nextjs'; // --- NEW ---
...
export default function ProductForm({
    ...
    const [currentImageUrl, setCurrentImageUrl] = useState(''); // --- NEW: Track existing URL ---
    ...
    useEffect(() => {
        if (initialData) {
            ...
            setCurrentImageUrl(initialData.image_url || ''); // --- NEW ---
    ...
    // ... (Keep helper functions: handleCollectionChange, handleVariantChange, addVariant, removeVariant, addTag, removeTag)
    ...
    const addTag = () => { if (tagInput && !tags.includes(tagInput.toLowerCase())) { setTags([...tags, tagInput.toLowerCase().trim()]); setTagInput(''); } };
    const removeTag = (tagToRemove) => setTags(tags.filter(tag => tag !== tagToRemove));

    // --- AI Handlers (Keep existing logic) ---
    const handleGenerateDescription = async () => {
        ...
        try {
            const response = await fetch('/api/generate-description', { method: 'POST', body: formData });
            if (!response.ok) throw new Error('Failed to generate description.');
            const data = await response.json();
            if (data.description) setDescription(data.description);
        } catch (error) { alert(`Error: ${error.message}`); } finally { setIsGeneratingDescription(false); }
    };

    const handleGenerateTagsClick = () => {
        if (!imageFile) return alert('Please select an image first.');
        if (!description || description.length < 20) setShowTagWarning(true); else generateTags();
    };
    ...
    const generateTags = async () => {
        ...
        try {
            const response = await fetch('/api/generate-tags', { method: 'POST', body: formData });
            if (!response.ok) throw new Error('AI tag generation failed');
            const data = await response.json();
            setTags(prev => [...new Set([...prev, ...data.tags])]);
        } catch (error) { alert(`Error: ${error.message}`); } finally { setIsGeneratingTags(false); }
    };
    ...
    const uploadImage = async (file) => {
        ...
        const { error: uploadError } = await supabase.storage
            .from('products') // Ensure you created this bucket in Supabase!
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const productData = {
                name,
                description,
                image_url: finalImageUrl, // --- NEW: Send URL ---
    ...
    // ... (Render logic remains mostly the same, just verifying inputs)
    return (
        <div className="bg-gray-800 p-6 rounded-lg mt-4 mb-8 space-y-6 border border-gray-700">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-md p-2" required />
                </div>
                ...
                <div>
                    <div className="flex justify-between items-end mb-1">
                        ...
                        <button type="button" onClick={handleGenerateDescription} disabled={!imageFile || isGeneratingDescription} className="text-xs bg-teal-600 hover:bg-teal-700 text-white py-1 px-2 rounded disabled:bg-gray-600">
                            {isGeneratingDescription ? 'Writing...' : '✨ Auto-Write with AI'}
                        </button>
                    </div>
                    <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-md p-2" rows="3"></textarea>
                </div>

                {/* SEO Fields (Hidden for brevity in this snippet, but should be there) */}
                {/* Taxonomy (Categories/Collections) */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label className="block text-sm font-medium mb-1">Category</label>
                        <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="w-full bg-gray-700 p-2 rounded-md border border-gray-600">
                        ...
                        <div className="max-h-32 overflow-y-auto bg-gray-900 p-3 rounded-md space-y-2 border border-gray-600">
                            {collections.map(collection => (
                                <div key={collection.id} className="flex items-center">
                                    <input type="checkbox" checked={selectedCollectionIds.includes(collection.id)} onChange={() => handleCollectionChange(collection.id)} className="h-4 w-4 bg-gray-700 rounded text-indigo-600"/>
                ...
                <div className="bg-gray-900 p-4 rounded-md border border-gray-700">
                    <div className="flex flex-col md:flex-row gap-4 mb-4">
                        <div className="flex-1">
                            ...
                            <input type="file" accept="image/*" onChange={(e) => setImageFile(e.target.files[0])} className="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-indigo-600 file:text-white cursor-pointer"/>
                            {currentImageUrl && !imageFile && (
                                <p className="text-xs text-green-400 mt-1">Current image loaded.</p>
                            )}
                        </div>
                        <button type="button" onClick={handleGenerateTagsClick} disabled={!imageFile || isGeneratingTags} className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-md disabled:bg-gray-600 self-end">
                            {isGeneratingTags ? 'Generating...' : '✨ Generate Tags'}
                        </button>
                    </div>
                    <div className="flex gap-2 mb-2">
                        <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addTag())} placeholder="Add tag..." className="flex-grow bg-gray-700 p-2 rounded-md border border-gray-600" />
                        <button type="button" onClick={addTag} className="bg-gray-600 px-4 rounded-md">Add</button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {tags.map(tag => (
                            <span key={tag} className="bg-blue-600 text-white text-sm px-2 py-1 rounded-full flex items-center">{tag}<button type="button" onClick={() => removeTag(tag)} className="ml-1 font-bold">&times;</button></span>
                        ))}
                    </div>
                </div>
