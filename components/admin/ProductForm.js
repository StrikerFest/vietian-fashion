...
export default function ProductForm({ initialData, categories = [], collections = [], onSuccess, onCancel }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (finalName !== name) {
                onSuccess(`Product published and "Generated" tag removed!`);
            } else {
        ...
        <div className={`bg-gray-800 p-6 rounded-lg transition-all ${
            ...
            {isGenerated && (
                <div className="bg-indigo-900/30 border border-indigo-500/50 text-indigo-200 px-4 py-3 rounded mb-6 flex items-start gap-3">
                    ...
                    <div>
                        <p className="font-bold text-sm">AI-Generated Draft</p>
                        <p className="text-xs opacity-80">This product was created by Gemini. Please review the tags, category, and price before setting status to <strong>Active</strong>.</p>
                    </div>
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-t border-gray-700 pt-6">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mb-2">Where does this live in the menu?</p>
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mb-2">Marketing groups (Summer, Featured)</p>
                    ...
                    <div>
                        <h3 className="font-semibold mb-2 text-purple-400">Attributes / Tags</h3>
                        <p className="text-xs text-gray-500 mb-2">Filterable traits (Vintage, Cotton)</p>
                        <div className="max-h-40 overflow-y-auto bg-gray-700 p-2 rounded border border-gray-600">
                ...
                <div className="border-t border-gray-700 pt-6 mt-6">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-bold text-white">SKUs & Pricing</h3>
                    </div>

                    <div className="bg-indigo-900/20 p-4 rounded border border-indigo-500/30 mb-6">
                        <p className="text-sm text-indigo-300 mb-2 font-bold">Configure Variant Options</p>
                        <p className="text-xs text-gray-400 mb-3">Check the attributes that define your variants (e.g. Size, Color).</p>
                ...
                <div className="flex justify-end gap-4 pt-6 border-t border-gray-700 sticky bottom-0 bg-gray-800 pb-2 z-10">
                    <button type="button" onClick={onCancel} className="px-6 py-2 rounded bg-gray-700 hover:bg-gray-600 text-white font-bold transition-colors">Cancel</button>
                    <button
                        type="submit"
                    ...
                    >
                        {isSubmitting ? 'Saving...' : (
                            isGenerated && status === 'active' ? 'Publish & Remove [G]' : 'Save Product'
                        )}
                    </button>
                </div>
            </form>
